...
async function testGroupBasicInfo(accessToken) {
  ...
  for (const fields of testFields) {
    try {
      ...
      console.log(`❌ Fields "${fields}" failed:`, error.response?.data?.error?.message || error.message);
    }
  }
...
async function testAlternativeApproaches(accessToken) {
  ...
  for (const version of versions) {
    try {
      ...
      console.log(`❌ Version ${version} failed:`, error.response?.data?.error?.message || error.message);
    }
  }
}
...
async function testPublicEndpoints() {
  ...
  for (const url of publicTests) {
    try {
      ...
      console.log(`❌ Public endpoint failed:`, error.response?.data?.error?.message || error.message);
    }
  }
}
