-- Fix users without default avatars and microphones for MySQL
...
INSERT INTO user_avatars (id, userId, baseAvatarId, avatarId, acquiredAt)
SELECT 
  UUID(),
  u.id,
  (SELECT id FROM avatars WHERE isFree = 1 AND isAvailable = 1 ORDER BY id LIMIT 1),
  (SELECT id FROM avatars WHERE isFree = 1 AND isAvailable = 1 ORDER BY id LIMIT 1),
  NOW()
FROM users u
LEFT JOIN user_avatars ua ON u.id = ua.userId
WHERE ua.userId IS NULL
  AND EXISTS (SELECT 1 FROM avatars WHERE isFree = 1 AND isAvailable = 1);
...
SELECT 
  UUID(),
  u.id,
  (SELECT id FROM microphones WHERE isFree = 1 AND isAvailable = 1 ORDER BY id LIMIT 1),
  NOW()
FROM users u
LEFT JOIN user_microphones um ON u.id = um.userId
WHERE um.userId IS NULL
  AND EXISTS (SELECT 1 FROM microphones WHERE isFree = 1 AND isAvailable = 1);

-- Step 4: Update users who don't have equipped avatars
UPDATE users 
SET equippedAvatarId = (SELECT id FROM avatars WHERE isFree = 1 AND isAvailable = 1 ORDER BY id LIMIT 1)
WHERE equippedAvatarId IS NULL
  AND EXISTS (SELECT 1 FROM avatars WHERE isFree = 1 AND isAvailable = 1);

-- Step 5: Update users who don't have equipped microphones  
UPDATE users 
SET equippedMicrophoneId = (SELECT id FROM microphones WHERE isFree = 1 AND isAvailable = 1 ORDER BY id LIMIT 1)
