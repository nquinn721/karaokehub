...
async function bootstrap() {
  ...
  const userMicrophoneRepository = app.get<Repository<UserMicrophone>>(
    getRepositoryToken(UserMicrophone),
  );
  ...
  for (const user of allUsers) {
    ...
    if (userHasAvatars === 0) {
      console.log(
        `👤 User ${user.stageName || user.name} (${user.id}) has no avatars, adding default...`,
      );
    ...
    if (userHasMicrophones === 0) {
      console.log(
        `👤 User ${user.stageName || user.name} (${user.id}) has no microphones, adding default...`,
      );
    ...
    if (!user.equippedAvatarId || !user.equippedMicrophoneId) {
      console.log(
        `👤 User ${user.stageName || user.name} (${user.id}) missing equipped items, setting defaults...`,
      );
