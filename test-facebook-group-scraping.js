...
async function testWebScraping() {
  ...
  try {
    ...
    const groupData = await page.evaluate(() => {
      ...
      // Try to find posts
      const postElements = document.querySelectorAll('[data-testid="post_message"], .userContent, [data-ad-preview="message"]');
      postElements.forEach((post, index) => {
        if (index < 3 && post.textContent.trim()) { // Get first 3 posts
