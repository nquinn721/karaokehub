...
const ApiLogsMonitor = observer(() => {
  ...
  useEffect(() => {
    fetchData();
  }, [hoursRange, filterLevel, filterType]);
          ...
          <CardContent>
            ...
            <Grid container spacing={2} sx={{ mt: 2 }}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                ...
                >
                  <Chip
                    label={itunesRateLimit?.circuitBreakerStatus || 'Unknown'}
                    color={
                      (itunesRateLimit?.circuitBreakerStatus || 'UNKNOWN') === 'CLOSED'
                        ? 'success'
                        : 'error'
                    }
                    size="small"
            ...
            {(itunesRateLimit?.rateLimitReachedCount || 0) > 0 && (
              <Alert severity="warning" sx={{ mt: 2 }}>
                <Typography variant="body2">
                  Rate limit reached {itunesRateLimit?.rateLimitReachedCount || 0} times today.
                  Current delay: {itunesRateLimit?.minDelayBetweenRequests || 0}ms between requests.
                </Typography>
              </Alert>
            )}
          </CardContent>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 0.5 }}>
                        ...
                        {log.circuitBreakerTripped && (
                          <Chip label="Circuit Breaker" color="error" size="small" />
                        )}
                        {log.statusCode && (
                          <Chip
