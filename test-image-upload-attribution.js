...
async function testImageUploadUserAttribution() {
  ...
  try {
    ...
    const vendorResponse = await axios.get(
      `${BASE_URL}/vendors/search?name=Test Vendor Upload`,
      {
    ...
    const venueResponse = await axios.get(
      `${BASE_URL}/venues?search=Test Venue Upload`,
      {
        headers: { 'Authorization': `Bearer ${token}` }
      }
    );
    ...
    // Check DJs
    const djResponse = await axios.get(
      `${BASE_URL}/djs?search=Test DJ Upload`,
      {
        headers: { 'Authorization': `Bearer ${token}` }
      }
    );
    ...
    const showResponse = await axios.get(
      `${BASE_URL}/shows?venue=Test Venue Upload&day=Monday`,
      {
        headers: { 'Authorization': `Bearer ${token}` }
      }
    );
